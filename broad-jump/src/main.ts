...
import { registerJumpDatasets } from "./bootstrap";
...
function initializeMobileExamples() {
  const select = document.getElementById("mobile-example-select") as HTMLSelectElement;
...
async function main() {
  try {
    ...
    setStatus("Loading JUMP metadata (0/11)...", "loading");
    await registerJumpDatasets(contextManager, ({ loaded, total, lastTable }) => {
      const suffix = lastTable ? ` - ${lastTable}` : "";
      setStatus(`Loading JUMP metadata (${loaded}/${total})${suffix}`, "loading");
    });
    ...
    setStatus("Ready - 11 tables loaded", "success");
